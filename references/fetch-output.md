...
| `lint` | Read-only drift report against the methodology (nothing is changed): `type_off_vocabulary` (`{number, type}` for every entry whose Type is not one of the six), `compound_domain` (possible compound Domains, for the owner to check: `{number, domain, separator}` for every Domain holding `/`, `,` or `&` — the three shapes named in #4; `separator` is the character found, because the Domain set is owner-defined and an `&` name such as the methodology's `Data & Metrics` may be one legal value, so the owner checks it against their set rather than repairing it), `related_without_number` (`{number, text, located}` for every `Related patterns` line with no `#N` — `located` is the entry whose full title the line contains, else the single entry whose title prefix before its dash it contains, else `null`; anything ambiguous is `null`), `related_dangling` (`{number, ref}` for every `#N` that resolves to no entry — a bare ticket number counts, and the owner rewrites that line), `reinforced_off_format` (`{number, line}` for every line that reads as a reinforcement header but is not `**Reinforced (YYYY-MM-DD)**: ` at the line start — bulleted, `Day N` or another qualifier inside the parentheses, `**Reinforced**:` with the date in the text, or the labels `Reinforcing event` / `Reinforcement`). Only `Issue #N` / `PR #N` / `MR #N` are recognised as tickets and ignored. A list item that wraps onto the next line is read as one item; a bold label, `---` or a heading ends the list. `summary` carries the counts (`type_off_vocabulary`, `compound_domain`, `related_lines`, `related_without_number`, `related_located`, `related_dangling`, `reinforced_off_format`); quote it in the run summary |
